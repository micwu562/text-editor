...
const editor = () => {
	 ...
	 * ignores whitespace at the start
	 * @param {number} pos
	 * @param {'forward' | 'back'} dir
	 */
	const _nextWordBoundary = (pos, dir) => {
		const di = dir === 'forward' ? 1 : -1
		const offset = dir === 'forward' ? 0 : 1 // when moving backword, correct cursor pos is +1.
		let wsOver = false
		let prevIsWord = null
	...
	const _moveCursor = (dir, by = 'char', select = false) => {
		switch (dir) {
			case 'left': {
				// if we're cancelling a selection, the cursor doesn't do movement!
				// only moves to endpoint of selection, depending on dir.
	...
	const draw = (timestamp) => {
		...
		/** @unused threshold line */
		// ctx.fillRect(
		// 	ctx.canvas.width / dpr - _P - mspfs.length,
		// 	ctx.canvas.height / dpr - _P - 50 - _CH_H * _LN_H,
		// 	mspfs.length,
		// 	1
		// )
		ctx.fillText(
